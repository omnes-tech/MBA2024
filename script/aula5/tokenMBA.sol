...
interface IERC20 {
    function totalSupply() external view returns (uint256); //numeros de tokens criados, com função externa que não gasta gas e retorna o valor

    function balanceOf(address tokenOwner) external view returns (uint256 balance); //saldo de tokens, com função externa que não gasta gas e retorna o valor do saldo

    function transfer(address to, uint256 tokens) external returns (bool success); // transferencia de token, com função externa que retorna booliana verdadeira ou falsa.
    ...
    function allowance(address tokenOwner, address spender)
        external
        view
        returns (uint256 remaining); //Permição do criador do token para o endereço que irá gastar o determinado token e a quantidade delimitada

    function approve(address spender, uint256 tokens) external returns (bool success); //aprovação do endereço gastador e envio dos tokens, com função externa que retorna booliana verdadeira ou falsa

    function transferFrom(
        address from,
        address to,
        uint256 tokens
    ) external returns (bool sucess); //treanferencia de um endereço para o outro os valores dos tokens, com função externa e retorna booliana verdadeira ou falsa

    event Transfer(address indexed from, address indexed to, uint256 tokens); //evento da transferencia do endereço que transferiu e do endereço que recebeu os tokens
    event Approval(address indexed tokenOwner, address indexed spender, uint256 tokens); //evento da aprovação com o endereço do dono do token o endereço gastador(comprador) e a quiantidade de tokens.

    // repare o padrão de de 6 funções e 2 eventos que é aceito universalmente nos ERC20 tokens.
}

contract MBAUnicap is
    IERC20 // vamos definir as variáveis estáveis de um ERC20
{
    ...
    uint256 public decimals = 0; //quantidade de decimals. Posteriormente iremos mudar para 18, que é o mais utilizado e aceito.
    uint256 public override totalSupply; // valor da quantidade de tokens criados, devemos utilizar a função override para conseguirmos falar com o interface ERC20 e sua função totalSupply
    ...
    mapping(address => uint256) public balances; //O mapping é relativo aos saldos das carteiras/endereços que receberam os tokens e seus valores, ou seja, guarda a quantidade/saldo de tokens que o endereço possui
    ...
    mapping(address => mapping(address => uint256)) allowed; //esse mapeamento inclui contas aprovadas para saques de uma determinada conta junto com o saque, alguns permitidos para cada uma delas.
    ...
    function balanceOf(address tokenOwner) public view override returns (uint256 balance) {
        //lembre-se que quando temos um interface devemos usar na função do contrato o override para conseguir implementa-lo
        return balances[tokenOwner]; //Dessa forma, retorna o saldo de tokens da conta inserida na função
    }

    function transfer(address to, uint256 tokens) public override returns (bool success) {
        //utilizamos a função de tranferir tokens para uma conta determinada nesta função
        require(
            balances[msg.sender] >= tokens,
            "voce nao tem tokens suficiente para realizar a transferencia"
        ); //condição de que a conta que esta mandando o token deve ter mais ou igual o numero de tokens que está querendo transferir, caso não tenha não será possível
    ...
    function allowance(address tokenOwner, address spender) public view override returns (uint256) {
        //função de autorização do dono dos tokens para o endereço que poderá gastar a quantidade de tokens delimitada
        return allowed[tokenOwner][spender]; //retorna os endereços do mapping allowed e endereços inseridos.
    }

    function approve(address spender, uint256 tokens) public override returns (bool success) {
        //função de aprovar a autorização de crédito dos tokens para o gastador
        require(
            balances[msg.sender] >= tokens,
            "Voce nao tem o numero suficiente de tokens para autorizar ao spender"
        ); //condição de quem for aprovar tem que ter o saldo em sua carteira de tokens igual ou maior ao inserido na função
        require(tokens > 0);
    ...
    function transferFrom(
        ...
        require(
            ...
            "A quantidade de tokens autorizados da conta from para a to deve ser maior que a quantidade de tokens aqui inserido, ou seja, insira somente um numero menor de tokens que a delimitada na autorizacao"
        ); //condição de que a autorização do endereço que vai autorizar(from) para o endereço que esta sendo autorizado(to), tenha um numero de tokens igual ou maior que o autorizado por este (from)
        require(
            balances[from] >= tokens,
            "o saldo da conta from deve ser maior ou igual a qauntidade de tokens aqui inseridos"
        ); // o saldo do endereço que está fazendo a transferencia tem que ter um numero de tokens igual ou maior que o inserido por ele.
    ...
    function mint(uint256 _quantity) public {
        //função que permite qualquer um mintar
