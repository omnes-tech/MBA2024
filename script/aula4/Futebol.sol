...
contract PartidaFutebol {
    ...
    mapping(string => uint256) public VitoriasCasa;
    mapping(string => uint256) public VitoriasVisitantes;

    function IniciarPartida(
        string memory _timeCasa,
        string memory _timeVisitante,
        uint256 _idPartida
    ) public {
    ...
    function encerrarPartida(uint256 _idPartida)
        public
        OnlyArbitro
        returns (
            uint256,
        ...
        )
    {
        ...
        uint256 placarCasa = idPartida[_idPartida].placarCasa;
        uint256 placarVisitante = idPartida[_idPartida].placarVisitante;
        //usamos a library para devolver o valor máximo dos placares
        uint256 vencedor = placarCasa.max(placarVisitante); //repare na library que retorna uma uint
    ...
    function consultarPlacar(uint256 _idPartida) public view returns (uint256, uint256) {
        return (idPartida[_idPartida].placarCasa, (idPartida[_idPartida].placarVisitante));
    }
